package org.example.model.PokePerks.Entity.TierOne;

import org.example.model.PokePerks.PokePerks;
import org.example.model.Pokemons.Pokemon;

import javax.swing.*;

public class PlusAttackTierOne extends PokePerks {

    private final int value = 2;
    public PlusAttackTierOne(Pokemon parent) {
        super(1, parent,"Atak + 2");
        icon=new ImageIcon("src/main/resources/Image/AttackTierOne.png");
    }

    @Override
    public void activatePerks() {
        if(checkRequirements()) {
            parent.increaseAttack(value);
            parent.subPerksPoint(1);
            isActive = true;
        }
    }

    @Override
    public void deactivatePerks() {
        parent.decreaseAttack(value);
        parent.addPerksPoint(1);
        isActive=false;
    }

    @Override
    public boolean checkRequirements() {
        if(parent.getPerksPoints()==1){
            return true;
        }
        return false;
    }
}
